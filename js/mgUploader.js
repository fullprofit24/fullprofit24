 ...
 * 	"inputName"			:"imgLoader",								// Имя инпутв
 ...
 * 	"event"			: true,								// Ивент начала загрузки
 * }
 */
function FileUploader(ioptions){
	...
	if(typeof this.options['handler']=="undefined")
		return null;
	if(typeof this.options['event']!="boolean")
	...
	if(this.CheckBrowser()){
		...
		if(this.options['event'])
			document.getElementById(this.options['inputId']).addEventListener('change',function(evt){
	...
	var that=this;
	this.start=function(){
		evt=document.getElementById(this.options['inputId']);
